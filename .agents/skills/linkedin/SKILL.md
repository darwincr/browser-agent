...
```bash
./browser-agent-cli submit "Use the pre-authenticated LinkedIn browser to research this company and summarize recent hiring or growth signals."
```

Submit a long task and poll it:

```bash
./browser-agent-cli submit --no-wait "Research these LinkedIn profiles and summarize relevant background."
...
```bash
./browser-agent-cli submit --file ./companies.csv "Use LinkedIn to research these companies and return a concise table."
```
