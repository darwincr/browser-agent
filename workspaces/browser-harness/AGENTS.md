...
Use the **screen-recording** skill (`start-recording` / `stop-recording`) when watching the browser makes a task easier to verify or debug — for example login flows, visual regressions, or navigation failures. Write recordings inside this workspace directory.
